export const formatResponse = (api, type, message) =>
  `${type}.${api}.${message}`;

export const formatSuccessResponse = (api, message) =>
  formatResponse(api, 'success', message);
export const formatErrorResponse = (api, message) =>
...
export const JoiObjectError = (schema, field) => {
  const invalid = formatErrorResponse(schema, `${field}.invalid`);
  return {
    ...joiGeneralError(schema, field),
    'object.base': invalid,
